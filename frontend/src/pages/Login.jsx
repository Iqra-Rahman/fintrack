    ...
    <div className='flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br '>
      ...
      <form onSubmit={onSubmitHandler}>
        {state === 'Sign Up' && (
          ...
          <input 
          onChange={e => setName(e.target.value)} value={name} className='bg-transparent outline-none' type='text' placeholder='Full Name' required/>
        </div>
        )}
        <div className='mb-4 flex items-center gap-3 border w-full rounded-full px-5 py-2.5 bg-[#333A5C]'>
          ...
          className='bg-transparent outline-none' type='mail' placeholder='Email id' required/>
        </div>
        <div className='mb-4 flex items-center gap-3 border w-full rounded-full px-5 py-2.5 bg-[#333A5C]'>
        ...
        <button className='w-full py-2.5 rounded-full bg-gradient-to-r from-pink-500 to-pink-900 text-white font-medium'>{state}</button>
      </form>
